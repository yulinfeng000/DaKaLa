...
def user_login():
    ...
    return jsonify({'msg': "登录成功", "code": 200, "trigger": trigger, "ck": ck_info, "token": token, "stuid": stuid, "config": config})
...
def user_info(stuid):
    ...
    if trigger is None:
        trigger = True
    return jsonify({"stuid": stuid, "trigger": trigger, "ck": ck_info, "config": stu_config, "code": 200})
...
def update_config(stuid):
    ...
    config = {
        ...
        'application_location': rf.get('application_location', origin_conf['application_location']),
        'application_reason': rf.get('application_reason', origin_conf['application_reason']),
        'application_start_time': rf.get('application_start_time', origin_conf['application_start_time']),
        'application_start_day': rf.get('application_start_day', origin_conf['application_start_day']),
        'application_end_day': rf.get('application_end_day', origin_conf['application_end_day']),
        'application_end_time': rf.get('application_end_time', origin_conf['application_end_time']),
        'scheduler_time_segment': rf.get('scheduler_time_segment', origin_conf['scheduler_time_segment']),
        'scheduler_start_time': rf.get('scheduler_start_time', origin_conf['scheduler_start_time']),
    }
    userdb.db_put_user_config(stuid, config)
